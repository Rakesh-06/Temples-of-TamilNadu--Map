/* Kindly go through the README.md...... theres a good concept behind this map......... I believe............!
...
var map, marker, templeInfowindow; //globally declaring

// Used the iniMap() function as given the Google maps api lessons and added  ViewModel function with the alterations upto my understandings.
// hoping my map looks good with a concept behind it "TAMILNADU TOURISM featured on famous TEMPLES"...........
// thanks for the additional google maps api lessons............
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById('map'), {
        center: { // this center lat-lng not the exact center, but the location of my home "hah...hah..haaaa!!! if zoomed to leve 21.... that's my home"
            lat: 9.952168,
            lng: 78.125244
        },
        zoom: 6,
...
function populateInfoWindow(marker, templeInfowindow) {
    ...
    }).fail(function(jqXHR) { //(CHANING). Used . fail() instead of .error() as suggested in the video....
...
var TempleViewModel = function() {
    ...
    self.selectTemple = function(templesData) { //click func() for the marker
    ...
    self.filterTemple = ko.computed(function() {
        ...
        return ko.utils.arrayFilter(self.templesData(), function(templeItem) {
            ...
            var visibleTemple = templeItem.markers;
            if (templeCase.indexOf(filter) >= 0) { // Prefferd indexOf() over stringStartsWith as suggested in some stackoverflow queries......
                if (visibleTemple) {
                    visibleTemple.setVisible(true); // Makes the searched place to be visible both on the list and the map.....
                }
                return true;
            } else {
                if (visibleTemple) {
                    visibleTemple.setVisible(false); // Makes other plaes and markers to hide its visibility.
                }
                return false;
            }
        });
